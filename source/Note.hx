...
class Note extends FlxSprite
{
	...
	public function new(_strumTime:Float, _noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?_warning:Bool = false)
	{
		super();
		...
		y -= 2000;
		strumTime = _strumTime + Std.int(Config.offset);
		if (strumTime < 0)
			strumTime = 0;
		...
		switch (daStage)
		{
			...
			loadGraphic(Paths.image('num3', 'preload'));
			setGraphicSize(Std.int(width * 0.7));
			updateHitbox();
			antialiasing = true;
		}
